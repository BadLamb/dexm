...
of having your site safe takedowns and downtime. The price of execution depends on the amount of checks specified by the 
...
but these can be made larger for a higher cost of hosting. Your app will run in a very tightly sandboxed Linux envoirement that won't allow any
...
- **CDN contracts**   
CDN contracts are a form of contract that hosts static content(webpages, photos, videos) and are cached by many edge nodes around the globe.
...
- **DB Contracts**   
DB contracts give you access to a public distributed KV DB. PublicDBs are a way of store data across nodes, all the data on it is public but it isn't 
a big problem, we have developed systems that allow to publicly sharing many normally private informations such as password hashes. 
